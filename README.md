Project Description: 
This project implements an Agentic Information Retrieval (IR) system based on Large Language Models (LLM). 
...
The system is able to manage complex information needs, assess the ambiguity of user queries, decompose them, plan multi-step retrieval strategies, and self-correct in the event of missing or contradictory information. 
Flow orchestration is performed using LangGraph, while the agentic logic is entrusted to a Qwen 14B Instruct model (quantized for execution on resource-constrained hardware such as Google Colab). 
System Architecture: 
...
Input: The user enters the initial query. 
Routing: The Validator analyzes the query and decides whether to route it to the Refiner or the Planner. 
Autonomous Processing: The designated module executes its task (with possible internal self-criticism loops). 
Final Evaluation: Execution returns to the Validator, which judges the results. If the output is satisfactory, the cycle ends; otherwise, it generates textual feedback and triggers a new refinement or planning cycle. 
...
1. The Validator (validator.py) acts as the Conductor (input router) and Judge (output). It queries the model by imposing a rigorous JSON output (via Pydantic) based on the calculation of runtime evaluation metrics (e.g., Ambiguity Score, Information Sufficiency). 
...
2. The Refiner (refiner.py) Module responsible for Autonomous Query Refinement. It does not perform database searches, but prepares the perfect query via tool-calling. It has an internal self-criticism loop to evaluate its queries before returning them to the Validator. Depending on the problem detected in the input query, the model autonomously chooses which tool to apply: 
...
3. The Planner (planner.py) is the operational heart of the system, responsible for Multi-Step Retrieval Planning.
It receives queries from the Refiner, classifies the required strategy, and actively accesses external search tools (vector databases, web search). 
...
Technology Stack and Setup Orchestration: langgraph (state management and conditional loops). 
LLM Interaction: langchain (tool binding and structured output). 
Model: Qwen3-14B-Instruct (GGUF Quantization Q4_K_M). 
Inference Engine: llama-cpp-python (compiled with CUDA support for full GPU offload). 
Hardware note: The 14 billion parameter, 4-bit quantized model was selected to fit within the VRAM (16 GB) limits of the NVIDIA T4 GPU provided by Google's free Colab tier, while providing superior logic and JSON formatting capabilities compared to 7B models.
...
7. But, X. et al. (2023). Query Rewriting for Retrieval-Augmented Large Language Models.
